#ifndef DVP_PLUGINS_H
#define DVP_PLUGINS_H
...
{
	void smoothtransition(autoware_msgs::Lane::Ptr& waypoints, uint32_t id, double lateral_transition_rate)
		{
    if (id >0 and id < waypoints->waypoints.size()-1){
      ...
      while (not end_process){
        ...
        double C = waypoints->waypoints[id-1].pose.pose.position.y -
                   waypoints->waypoints[id-1].pose.pose.position.x * std::tan(yaw0);
        ...
        if (lateral_diff <= lateral_allowed){
          ...
          double sign = yaw0 > yaw1 ? -1:1;
          double dy = sign * lateral_allowed / std::cos(yaw0);
          double Vx = waypoints->waypoints[id].pose.pose.position.x - waypoints->waypoints[id-1].pose.pose.position.x;
          double Vy = waypoints->waypoints[id].pose.pose.position.y - waypoints->waypoints[id-1].pose.pose.position.y - dy;
          double t = Vx * std::sin(yaw0) + Vy * std::cos(yaw0);
          waypoints->waypoints[id].pose.pose.position.x = waypoints->waypoints[id-1].pose.pose.position.x + t * std::sin(yaw0);
          waypoints->waypoints[id].pose.pose.position.y = waypoints->waypoints[id-1].pose.pose.position.y + t * std::cos(yaw0) + dy;
          id++;
  ...
  bool shiftWaypoint(autoware_msgs::Lane::Ptr& waypoints, uint32_t id, double shift)
  {
  ...
  bool updateQuaternion(autoware_msgs::Lane::Ptr& waypoints, uint32_t id)
  {
    if (id < waypoints->waypoints.size()-1){
